...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response?.data?.user) {
        const userData = response.data.user;
        const token = response.data.token || response.data.access_token; // ✅ Asegúrate de obtener el token

        // ✅ Guardamos usuario Y token globalmente en el contexto
        ...
        // ✅ Redirección inteligente - verifica si venía de una ruta protegida
